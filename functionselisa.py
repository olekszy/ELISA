...
def cuttable(x, wave):
    ...
    try:
        ...
        con = input(wave +" table not defined. Do you want to continue? y or n ")
        if not con in ['y', 'Y', 'yes', 'Yes', 'YES']:
            exit()
    #name = z.iloc[row,1]
...
def xsamples(temp,df,popt):
    ...
    for i in samplelist: #loop over sample names extracted before
        ...
        temporary = pd.DataFrame({"Sample":[i], "Concentration":[z.real],"QC":[qc],"Meanread":[meanread]}) #create temporary table for better appending
...
def ODbyELISA(results):
    ...
    print(z)
    const = z.flat[3] #extract value
    conc = results['Meanread'].tolist() #change column to list
    lista = [] #create list for append
    #print(const)
    for i in conc:    
        x = (i/const)*1000 #formula over all rows
        lista.append(x)
...
def createimage(x,y,xnew,f,f2,r2valuestr, file):
        fig = plt.figure()
        plt.plot(x, y, 'o', xnew, f(xnew), '-', xnew, f2(xnew), '--')
        plt.title(file+" "+"4PL curve", fontdict=None, loc='center', pad=None)
        plt.legend(['data', 'linear', 'cubic'], loc='best')
        plt.xlabel("Concentration")
        plt.ylabel("OD")
        plt.text(2, 0.65,"R2 value" + r2valuestr)
        return fig
################################## PROGRAM STARTS HERE #################################################
def analysis(csv,platetemplate,standardsnumber,value,dil,filename): 
    ...
    for i in range(standardsnumber-1):
    ...
    z = createimage(x, y, xnew,f,f2,r2valuestr, csv)
